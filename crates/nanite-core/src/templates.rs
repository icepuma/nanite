...
impl TemplateRepository {
    ...
    pub fn output_names(&self) -> Vec<String> {
        self.bundles
            .iter()
            .flat_map(|bundle| bundle.templates.iter().map(|template| template.output_name.clone()))
            .collect::<BTreeSet<_>>()
            .into_iter()
            .collect()
    }
...
impl TemplateBundle {
    ...
    fn prepare_with_values(
        ...
        for placeholder in self.text_placeholders() {
            if values.contains_key(&placeholder.name) {
                prompted.insert(placeholder.name.clone());
                continue;
            }
            if prompted.insert(placeholder.name.clone()) {
    ...
    fn text_placeholders(&self) -> Vec<TextPlaceholder> {
        self.templates
            .iter()
            .flat_map(|template| template.text_placeholders())
            .collect()
    }
}
    ...
    pub fn prepare(&self, prompter: &mut impl Prompter) -> Result<PreparedTemplate> {
        ...
        .map(|bundle| {
            bundle
                ...
                .into_iter()
                .next()
                .expect("single bundle should contain one prepared template")
        })
    }
...
impl PreparedTemplate {
            ...
            match fragment {
                ...
                TemplateFragment::Ai(placeholder) => {
                    let readme_role = readme_roles
                        ...
                        .copied();
                    let label = if let Some(role) = readme_role {
                        readme_index += 1;
                        role.label().to_owned()
                    } else if let Some(heading) = last_heading.clone() {
                        heading
                    } else {
    ...
    pub fn build_context_bundle(&self, cwd: &Utf8Path) -> ContextBundle {
        ...
        let facts = build_repo_context_facts(cwd, repo_name.clone());
        ...
        summary_lines.push(format!(
            ...
            if facts.ci_workflows.is_empty() {
                "(none)".to_owned()
            } else {
                facts.ci_workflows.join(", ")
            }
        ));
        summary_lines.push(format!(
        ...
        summary_lines.push(format!(
            "- Docs present: {}",
            if facts.docs_present.is_empty() {
                "(none)".to_owned()
            } else {
                facts.docs_present.join(", ")
            }
        ));
        summary_lines.push(format!(
            "- Workspace inventory: {}",
            if facts.workspace_inventory.is_empty() {
                "(none)".to_owned()
            } else {
                facts.workspace_inventory.join("; ")
            }
        ));
        ...
        for candidate in [
            ...
            if let Some(snippet) = read_context_snippet(cwd.join(candidate), candidate) {
    ...
    pub fn verify_readme(
        ...
        for fragment in &fragments {
            ...
            match fragment.readme_role {
                Some(ReadmeFragmentRole::Badges) => {
                ...
                Some(ReadmeFragmentRole::Overview) => {
                    ...
                    let sentence_count = count_sentences(value);
                ...
                Some(ReadmeFragmentRole::QuickStart) => {
        ...
        let expected_sections = self.expected_static_readme_sections(ai_values);
            ...
            match fragment {
                ...
                TemplateFragment::Ai(placeholder) => {
                    if placeholder.index == active_fragment {
                        ...
                        rendered.push_str(&format!("{{{{ai:{}}}}}", placeholder.prompt));
                    }
                }
            }
        }
...
fn build_template_engine(values: &BTreeMap<String, String>) -> Result<Tera> {
...
impl TeraFunction for CurrentYearFunction {
    fn call(&self, _args: &std::collections::HashMap<String, JsonValue>) -> tera::Result<JsonValue> {
...
fn read_context_snippet(path: Utf8PathBuf, display_path: &str) -> Option<ContextSnippet> {
    if !path.is_file() {
        return None;
    }
...
fn read_text(path: Utf8PathBuf) -> Option<String> {
    fs::read_to_string(path.as_std_path()).ok()
}

fn read_json(path: Utf8PathBuf) -> Option<JsonValue> {
...
fn detect_named_target(path: Utf8PathBuf, names: &[&str]) -> Option<String> {
...
fn parse_readme_document(markdown: &str) -> ParsedReadme {
    ...
    let first_non_empty_is_h1 = first_non_empty
...
impl ReadmeVerificationReport {
    ...
    pub fn render_messages(&self) -> Vec<String> {
        self.findings
            .iter()
            .map(|finding| match &finding.fragment_label {
                Some(label) => format!("{label}: {}", finding.message),
...
mod tests {
    ...
    fn parses_filename_frontmatter_and_lists_templates() {
        ...
        .unwrap();
        fs::write(root.join("default/LICENSE"), "---\nfilename: LICENSE\n---\nMIT\n").unwrap();
